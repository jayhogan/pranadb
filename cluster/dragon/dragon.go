...
func (d *Dragon) sendPropose(shardID uint64, request []byte) (uint64, []byte, error) {
	...
	// TODO eliminate duplicated code with executeRead
	...
	resp, err := requestClient.SendRequest(clusterRequest, 10*time.Second, serverAddresses...)
	if err != nil {
		return 0, nil, err
	}
...
func (d *Dragon) executeRead(shardID uint64, request []byte) ([]byte, error) {
	...
	// This node doesn't have a replica for this shard - we need to find a node that does - any node will work
	// but the leader is preferable for perf reasons
	nids, ok := d.shardAllocs[shardID]
	if !ok {
		panic(fmt.Sprintf("can't find shard allocs for shard %d", shardID))
	}
	// TODO sort with leader first - this could give better perf - currently we don't know the leader
	ln := len(nids)
	serverAddresses := make([]string, ln, ln)
...
func (d *Dragon) getOrCreateRequestClient(shardID uint64) (remoting.Client, error) {
	...
	if client == nil {
		client = remoting.NewClient(d.cnf.NotifierHeartbeatInterval)
