...
func TestSendRequest(t *testing.T) {
	...
	client := newClient(heartbeatInterval)
	err = client.Start()
	require.NoError(t, err)
	defer stopClient(t, client)
	...
	resp, err := client.SendRequest(req, 10*time.Second, "localhost:7888")
	require.NoError(t, err)
...
func TestSendRequestWithError(t *testing.T) {
	...
	_, err = client.SendRequest(req, 10*time.Second, "localhost:7888")
	require.Error(t, err)
